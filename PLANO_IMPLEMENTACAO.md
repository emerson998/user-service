...
### Por que o `usuarios-service` é o piloto (contexto de negócio)

> Ver [`../PIX_SIMULATOR_ARQUITETURA.md`](../PIX_SIMULATOR_ARQUITETURA.md)
> para o mapa completo da plataforma (PIX Simulator). O `usuarios-service`
> não é mais só "um piloto escolhido para uma demo" — é **literalmente o
> serviço de identidade da Fase 1**: o futuro `dict-service` (diretório de
> chaves) vai consultar este serviço para validar o titular antes de
> registrar ou resolver uma chave PIX.

O `usuarios-service` administra perfis e dados cadastrais dos usuários —
uma dependência **upstream** de fluxos críticos de negócio, não só mais um
CRUD isolado. Exemplo concreto: antes de efetivar um PIX, o fluxo de
pagamentos (o futuro `dict-service`/motor de liquidação, ver documento
de arquitetura) precisa validar o perfil/titularidade do pagador
consultando `GET /api/v1/users/{id}` aqui. Se essa chamada falha — como no
incidente simulado pelo chaos (seção 3) — a resolução da chave PIX não
confirma o titular, e a transação fica bloqueada. Não é um bug no PIX; é
uma dependência upstream quebrada se propagando adiante.

É por isso que o `usuarios-service` é a primeira peça real da Fase 1 do
fluxo "Do Bug ao PR" (ver `chaos-fix-agent/PLANO_IMPLEMENTACAO.md`): uma
falha aqui bloqueia diretamente uma etapa crítica da jornada de Pessoa
Física (PF), o que torna o incidente — e a velocidade da correção
automática — visível para o negócio, não só para quem olha log de
aplicação. Ver seção 3.9 para como demonstrar esse "elo" com o PIX ao
vivo, enquanto o `dict-service` de verdade não existe.

### Serviços envolvidos no piloto

Este plano descreve só o `usuarios-service`. Para deixar claro o que cada
peça do cenário é — real, planejada, ou só uma abstração para a
demonstração — segue o mapa local do piloto (o mapa **completo** de toda a
plataforma PIX Simulator, incluindo os serviços ainda não construídos
como `dict-service`, `settlement-engine`, `traffic-agents` e
`chaos-agent`, vive em
[`../PIX_SIMULATOR_ARQUITETURA.md`](../PIX_SIMULATOR_ARQUITETURA.md)):

| Serviço | O que é | Stack | Status neste piloto |
|---|---|---|---|
| **usuarios-service** | CRUD de usuários; hospeda o bug controlado (seção 3) e a notificação do incidente (seção 3.8) | Java / Spring Boot | Real — é este repositório, plano completo aqui |
| **chaos-fix-agent** | Recebe a notificação, diagnostica a causa raiz, gera a correção, abre o PR e notifica via WhatsApp | Python / FastAPI | Real — repositório irmão `../chaos-fix-agent`, plano completo em `chaos-fix-agent/PLANO_IMPLEMENTACAO.md` |
| **Bot de chaos** | Cliente HTTP que chama `POST /internal/chaos/users-flow/trigger` para provocar o incidente | Script (curl/Postman/CI job) | Real, mas simples — não é um serviço, é o "ator" que inicia a demo |
| **Fluxo de PIX** | Consumidor upstream do `usuarios-service`: valida titularidade (`GET /api/v1/users/{id}`) antes de efetivar uma transferência | — | **Abstração para a demo** — não existe como serviço de verdade neste piloto; representado por um `curl` narrado ou um `pix-service-stub` mínimo (seção 3.9), só para tornar o bloqueio de negócio visível |
| **Pipeline de CI/CD** | Builda, testa e promove a correção ao ambiente de destino depois do merge do PR | Já existente (ex.: GitHub Actions) | **Já existe** fora deste piloto — o `chaos-fix-agent` só entrega um PR mergeável, não constrói nem aciona esse pipeline |

A regra que separa "real" de "abstração" aqui: qualquer coisa que
efetivamente lê/escreve código ou dado (usuarios-service, chaos-fix-agent,
bot de chaos) é implementada de verdade neste piloto. O PIX entra só como
**consequência de negócio narrada** — o objetivo é mostrar o efeito, não
construir um segundo domínio (pagamentos) só para a demo.
...
**Observabilidade** (requisito de `PIX_SIMULATOR_ARQUITETURA.md` seção 6):
adicionar `spring-boot-starter-actuator` ao `pom.xml`, expondo
`/actuator/health` e `/actuator/metrics` — o Spring Boot já entrega isso
quase de graça, sem acoplar a nenhuma stack específica de observabilidade.
...
### 3.9 Elo com o fluxo de PIX (blast radius para a demonstração)

Para deixar o impacto de negócio visível na apresentação — não só "um 500
em um endpoint qualquer" — sem precisar construir um `pix-service`
completo:

- **Contrato assumido**: antes de efetivar um PIX, o fluxo de pagamentos
  chama `GET /api/v1/users/{id}` no `usuarios-service` para validar que o
  pagador existe e está com o perfil ativo (`enabled = true`) — uma
  checagem de titularidade de PF antes de mover dinheiro.
- **Opção rápida para o ensaio** (recomendada para a primeira demo): sem
  subir nenhum serviço novo, simular essa chamada ao vivo com um
  `curl`/Postman apontando direto para `GET /api/v1/users/{id}`, narrando
  *"isso é exatamente o que o pix-service chamaria antes de confirmar o
  PIX"* — antes do chaos, retorna 200 (PIX seguiria); depois do `trigger`,
  retorna 500 (PIX ficaria bloqueado).
- **Opção mais completa** (se quiserem uma segunda tela reagindo ao vivo):
  um `pix-service-stub` minúsculo, um único endpoint
  `POST /pix/transfer` que faz esse `GET /api/v1/users/{id}` e devolve
  `"PIX confirmado"` ou `"PIX bloqueado: não foi possível validar o
  pagador"` conforme a resposta do `usuarios-service` — sem regra de
  negócio de PIX de verdade, só o suficiente para mostrar o efeito
  encadeado em outra aplicação.
- Em ambos os casos, o roteiro da demo ganha uma frase concreta: *"o PIX
  do cliente X está bloqueado porque o usuarios-service não consegue
  validar o perfil dele"* — e, depois do PR aprovado e promovido (ver
  `chaos-fix-agent/PLANO_IMPLEMENTACAO.md`, seção 3), *"o PIX volta a
  funcionar assim que o usuarios-service volta ao ar"*.
